...
command = """ALTER TABLE  cu_log ADD  cul_actor BIGINT UNSIGNED DEFAULT 0 NOT NULL, ADD INDEX cul_actor_time (cul_actor, cul_timestamp);"""
...
def check(db):
    if 'cu_log' not in db.run_sql('show tables;'):
        return True
    return 'cul_actor' in db.run_sql('desc cu_log;')
