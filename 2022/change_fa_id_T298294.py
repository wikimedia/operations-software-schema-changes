...
command = """ALTER TABLE  /*_*/filearchive CHANGE  fa_id fa_id INT UNSIGNED AUTO_INCREMENT NOT NULL;"""
...
def check(db):
    query_res = db.run_sql('desc filearchive;')
    if not query_res:
        # Dry run
        return True
    field_def = query_res.split('fa_id')[1].split('\n')[0]
