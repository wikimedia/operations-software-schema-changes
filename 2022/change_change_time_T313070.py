...
command = """ALTER TABLE  /*_*/wb_changes CHANGE  change_time change_time BINARY(14) NOT NULL;"""
...
def check(db):
    query_res = db.run_sql('desc wb_changes;')
    if not query_res:
        # Dry run
        return True
    field_def = query_res.split('change_time')[1].split('\n')[0]
