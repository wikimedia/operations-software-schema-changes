...
command = """ALTER TABLE page CHANGE page_touched page_touched BINARY(14) NOT NULL;"""
...
def check(db):
    query_res = db.run_sql('desc page;')
    if not query_res:
        # Dry run
        return True
    field_def = query_res.split('page_touched')[1].split('\n')[0]
