...
command = """ALTER TABLE /*_*/page_restrictions CHANGE pr_type pr_type VARBINARY(60) NOT NULL,CHANGE pr_level pr_level VARBINARY(60) NOT NULL;"""
...
def check(db):
    query_res = db.run_sql('desc page_restrictions;')
    if not query_res:
        # Dry run
        return True
    field_def = query_res.split('pr_type')[1].split('\n')[0]
    return 'varbinary' in field_def.lower()
