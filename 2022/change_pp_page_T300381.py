...
command = """ALTER TABLE  page_props CHANGE  pp_page pp_page INT UNSIGNED NOT NULL;"""
...
def check(db):
    query_res = db.run_sql('desc page_props;')
    if not query_res:
        # Dry run
        return True
    field_def = query_res.split('pp_page')[1].split('\n')[0]
