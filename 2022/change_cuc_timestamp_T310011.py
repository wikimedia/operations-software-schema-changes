...
command = """ALTER TABLE  cu_changes CHANGE  cuc_timestamp cuc_timestamp BINARY(14) NOT NULL;"""
...
def check(db):
    query_res = db.run_sql('desc cu_changes;')
    if not query_res:
        # Dry run
        return True
    field_def = query_res.split('cuc_timestamp')[1].split('\n')[0]
