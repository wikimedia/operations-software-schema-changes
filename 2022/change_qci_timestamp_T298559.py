...
command = """ALTER TABLE querycache_info CHANGE qci_timestamp qci_timestamp BINARY(14) DEFAULT '19700101000000' NOT NULL;"""
...
def check(db):
    query_res = db.run_sql('desc querycache_info;')
    if not query_res:
        # Dry run
        return True
    field_def = query_res.split('qci_timestamp')[1].split('\n')[0]
