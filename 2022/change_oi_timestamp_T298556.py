...
command = """ALTER TABLE oldimage CHANGE oi_timestamp oi_timestamp BINARY(14) NOT NULL;"""
...
def check(db):
    query_res = db.run_sql('desc oldimage;')
    if not query_res:
        # Dry run
        return True
    field_def = query_res.split('oi_timestamp')[1].split('\n')[0]
