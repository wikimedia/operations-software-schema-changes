...
command = """ALTER TABLE protected_titles CHANGE pt_timestamp pt_timestamp BINARY(14) NOT NULL;"""
...
def check(db):
    query_res = db.run_sql('desc protected_titles;')
    if not query_res:
        # Dry run
        return True
    field_def = query_res.split('pt_timestamp')[1].split('\n')[0]
